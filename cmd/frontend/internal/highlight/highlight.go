...
func Code(ctx context.Context, p Params) (h template.HTML, aborted bool, err error) {
	...
	if ctx.Err() == context.DeadlineExceeded {
		...
		case gosyntect.ErrRequestTooLarge:
			expectedProblem = "request_too_large"
		case gosyntect.ErrPanic:
			expectedProblem = "panic"
		case gosyntect.ErrHSSWorkerTimeout:
			expectedProblem = "hss_worker_timeout"
		}
		if expectedProblem != "" {
			...
			// user an error.
			trace.Log(otlog.Bool(expectedProblem, true))
			errCollector.Collect(&err)
			prometheusStatus = expectedProblem
			table, err := generatePlainTable(code)
